...
example {x : ℤ} (hx : x ≥ 9) : x ^ 3 - 8 * x ^ 2 + 2 * x ≥ 3 :=
  calc
    x ^ 3 - 8 * x ^ 2 + 2 * x = (x ^ 2 + 2) * (x - 8) + 16 := by ring
    _ >= (9 ^ 2 + 2) * (9 - 8) + 16 := by rel [hx]
    _ >= 3 := by numbers

example {n : ℤ} (hn : n ≥ 10) : n ^ 4 - 2 * n ^ 2 > 3 * n ^ 3 :=
  calc
    n ^ 4 - 2 * n ^ 2
      = n ^ 2 * (n + 1) * (n - 4) + 2 * n ^ 2 + 3 * n ^ 3 := by ring
    _ >= n ^ 2 * (n + 1) * (10 - 4) + 2 * n ^ 2 + 3 * n ^ 3 := by rel [hn]
    _ = n ^ 2 * (n + 1) * 6 + 2 * n ^ 2 + 3 * n ^ 3 := by ring
    _ >= 10 ^ 2 * (10 + 1) * 6 + 2 * 10 ^ 2 + 3 * n ^ 3 := by rel [hn]
    _ > 3 * n ^ 3 := by extra

example {n : ℤ} (h1 : n ≥ 5) : n ^ 2 - 2 * n + 3 > 14 :=
  calc
    n ^ 2 - 2 * n + 3 = (n - 3) * (n + 1) + 6 := by ring
    _ >= (5 - 3) * (n + 1) + 6 := by rel [h1]
    _ = 2 * (n + 1) + 6 := by ring
    _ >= 2 * (5 + 1) + 6 := by rel [h1]
    _ > 14 := by numbers
